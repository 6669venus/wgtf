...
class WotPerformanceTestResult( SQLAlchemyBase ):
	...
	def __init__( self, changelist, executable, replayName , peakAllocatedMB, totalMemoryAllocations, loadTimeSec, frameTimeMs, successState, branchName):
		self.changelist = changelist
		self.executable = executable
	...
	def __repr__( self ):
		return "<WotPerformanceTestResult('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s') : hostname=%s" % \
			(self.changelist, self.executable, self,replayName, self.peakAllocatedMB, self.totalMemoryAllocations, self.loadTimeSec, self.frameTimeMs, 
			self.successState, self.branchName, self.hostname)


	@staticmethod
	def addResult( session, changelist, executable, replayName, peakAllocatedMB, totalMemoryAllocations, loadTimeSec, frameTimeMs, successState, branchName ):
		v = WotPerformanceTestResult( changelist, executable, replayName, peakAllocatedMB, totalMemoryAllocations, loadTimeSec, frameTimeMs, successState, branchName )
		session.add( v )
		session.commit()
		return v
