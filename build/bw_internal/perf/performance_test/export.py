...
from performance_test.database import ReferenceValue, FrameRateResult, LoadTimeResult, SmokeTestResult, LoadTimeReferenceValue, WotPerformanceTestResult, SmokeTestReferenceValue
...
class OutputColumn:
	def __init__( self, dbSession, hostname, configuration, executable, branchName, benchmarkName, name, value ):
		...
		if EXPORT_TYPE is not WotPerformanceTestResult:
		...
		self.rows = dbSession.query( EXPORT_TYPE ).filter_by(
						**queryFilter ). \
						order_by( EXPORT_TYPE.changelist ). \
						all()
						
		#self.outputData = [ (x.date, x.value) for x in self.rows ]
		self.outputData = [ (x.changelist, getattr(x, value)) for x in self.rows ]
		...
		self.changeValues = [x.changelist for x in self.rows ]	
		self.minChange = min( self.changeValues ) if self.changeValues else 0
		self.maxChange = max( self.changeValues ) if self.changeValues else 0
...
def main():
	...
	for column in columns:
		outputColumn = {
			"minChange": column.minChange,
			"maxChange": column.maxChange,
			"changeValues": column.changeValues,
			"name": column.name,
		...
		if column.minChange < minChange or minChange is 0:
			minChange =  column.minChange
		if column.maxChange > maxChange:
			maxChange =  column.maxChange 
		for change in column.changeValues:
	...
	output = {
		"dateMin": minDate,
		"dateMax": maxDate,
		"minChangelist": minChange,
		"maxChangelist": maxChange,
