...
def main( testType ):
	...
	parser.add_option( "-g", "--submit_to_graphite",
					...
					help = "Submit stats to graphite host: " + GRAPHITE_HOST )
	...
	# Main test
	try:
		testModule.run( buildConfig, exeType, dbSession, reportHolder,
			options.branch_tag, testName, options.compiled_space,
			options.changelist, options.primer_run, options.submit_to_graphite,
			flags )
