...
FUNCTION( BW_SET_BINARY_DIR _PROJNAME _DIRNAME )
	...
	IF (BW_PLATFORM_MAC)
		SET_TARGET_PROPERTIES( ${_PROJNAME} PROPERTIES
			LIBRARY_OUTPUT_DIRECTORY_DEBUG "${_DIRNAME}"
			PDB_OUTPUT_DIRECTORY_DEBUG "${_DIRNAME}"
		)
		SET_TARGET_PROPERTIES( ${_PROJNAME} PROPERTIES
			LIBRARY_OUTPUT_DIRECTORY_HYBRID "${_DIRNAME}"
			PDB_OUTPUT_DIRECTORY_HYBRID "${_DIRNAME}"
		)
		SET_TARGET_PROPERTIES( ${_PROJNAME} PROPERTIES
			LIBRARY_OUTPUT_DIRECTORY_CONSUMER_RELEASE "${_DIRNAME}"
			PDB_OUTPUT_DIRECTORY_CONSUMER_RELEASE "${_DIRNAME}"
		)
	ENDIF()
...
MACRO( BW_ADD_TEST _PROJNAME )
	ADD_TEST( NAME ${_PROJNAME}
		...
		WORKING_DIRECTORY ${BW_GAME_DIR}/unit_tests/${BW_PLATFORM})
...
MACRO( BW_ADD_TOOL_PLUGIN _PROJNAME _DIRNAME )
	...
	IF (BW_PLATFORM_MAC)
		BW_COPY_PLUGIN_TO_BUNDLE( ${_PROJNAME}  ${_DIRNAME} )
	ENDIF()
ENDMACRO()
...
FUNCTION( BW_COPY_PLUGIN_TO_BUNDLE _FROM_TARGET _DIRNAME )
	# Temporary copy to both Debug and Hybrid bundles
	ADD_CUSTOM_COMMAND(
		TARGET ${_FROM_TARGET}
		POST_BUILD
		COMMAND ${CMAKE_COMMAND} -E copy_if_different
			$<TARGET_FILE:${_FROM_TARGET}>
			${BW_GAME_DIR}/${_DIRNAME}/${BW_PLATFORM}/${BW_BUNDLE_NAME}_d.app/Contents/PlugIns/plugins/$<TARGET_FILE_NAME:${_FROM_TARGET}>
		COMMENT "Copying from ${_FROM_TARGET} to ${BW_GAME_DIR}/${_DIRNAME}/${BW_PLATFORM}/${BW_BUNDLE_NAME}_d.app/Contents/PlugIns/plugins."
		VERBATIM
	)
	ADD_CUSTOM_COMMAND(
		TARGET ${_FROM_TARGET}
		...
		COMMAND ${CMAKE_COMMAND} -E copy_if_different
			$<TARGET_FILE:${_FROM_TARGET}>
			${BW_GAME_DIR}/${_DIRNAME}/${BW_PLATFORM}/${BW_BUNDLE_NAME}.app/Contents/PlugIns/plugins/$<TARGET_FILE_NAME:${_FROM_TARGET}>
		COMMENT "Copying from ${_FROM_TARGET} to ${BW_GAME_DIR}/${_DIRNAME}/${BW_PLATFORM}/${BW_BUNDLE_NAME}.app/Contents/PlugIns/plugins."
		VERBATIM
	)
ENDFUNCTION()
...
MACRO( BW_CUSTOM_COPY_TO_PROJECT_OUTPUT _TARGET_DIR _RESOURCES )
    FOREACH( resFile ${_RESOURCES} )
            ...
            MESSAGE( "Configration for copying ${resFile} to ${_TARGET_DIR}" )
			ADD_CUSTOM_COMMAND( TARGET ${PROJECT_NAME} POST_BUILD
