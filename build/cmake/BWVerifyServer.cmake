...
ELSEIF( "${BW_LINUX_CONN_TYPE}" MATCHES "DELTACOPY" )
	FIND_PROGRAM( BW_LINUX_DELTACOPY_RSYNC_PATH "rsync.exe" 
		HINTS "${BW_SOURCE_DIR}/third_party/deltacopy" )
	FIND_PROGRAM( BW_LINUX_DELTACOPY_SSH_PATH "ssh.exe" 
		HINTS "${BW_SOURCE_DIR}/third_party//deltacopy" )
		
	SET( BW_LINUX_SSH_PATH ${BW_LINUX_DELTACOPY_SSH_PATH} )
	SET( BW_LINUX_RSYNC_PATH ${BW_LINUX_DELTACOPY_RSYNC_PATH} )
	
	SET( BW_LINUX_SSH_ARGS "-oUserKnownHostsFile=/dev/null" )
	SET( BW_LINUX_SSH_ARGS "${BW_LINUX_SSH_ARGS} -oStrictHostKeyChecking=no" )
	SET( BW_LINUX_SSH_ARGS "${BW_LINUX_SSH_ARGS} -oBatchMode=yes" )
	
	IF( NOT "${BW_LINUX_PRIV_KEY}" STREQUAL "" )
		SET( BW_LINUX_SSH_ARGS "${BW_LINUX_SSH_ARGS} -i \"${BW_LINUX_PRIV_KEY}\"" )
	ENDIF()
	SET( BW_LINUX_SSH_ARGS "${BW_LINUX_SSH_ARGS} ${BW_LINUX_USER}@${BW_LINUX_HOST}" )
	
	SET (BW_LINUX_RSYNC_ARGS "--chmod=a=rw,Da+x --delete --delete-excluded")
	SET (BW_LINUX_RSYNC_ARGS "${BW_LINUX_RSYNC_ARGS} -azvv")
	SET (BW_LINUX_RSYNC_ARGS "${BW_LINUX_RSYNC_ARGS} -e \"'${BW_LINUX_SSH_PATH} ${BW_LINUX_SSH_ARGS}\"")
	SET (BW_LINUX_RSYNC_ARGS "${BW_LINUX_RSYNC_ARGS} --exclude-from ${BW_SOURCE_DIR}/build/rsync_exclude_common.txt")
	SET (BW_LINUX_RSYNC_ARGS "${BW_LINUX_RSYNC_ARGS} --exclude-from ${BW_SOURCE_DIR}/build/rsync_exclude_build.txt")
...
IF( BW_REPLACE_LINUX_DIR )
	...
	# Quoting differs between Putty and DeltaCopy, possibly a bug with either.
	IF( "${BW_LINUX_CONN_TYPE}" STREQUAL "PUTTY_SSH" )
