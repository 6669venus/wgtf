...
BW::string Item::displayName()
{
	...
	return displayName_.empty() ? name_ : Manager::instance().functors().text( displayName_, ItemPtr( this ) );
}
...
BW::string Item::description()
{
	...
	return description_.empty() ? displayName() : Manager::instance().functors().text( description_, ItemPtr( this ) );
}
...
unsigned int Item::update()
{
	...
	if( types().find( type() ) != types().end() )
		return types()[ type() ]->update( ItemPtr( this ) );
...
bool Item::processInput( InputDevice* inputDevice )
{
	...
	{
		if( types().find( type() ) != types().end() )
			types()[ type() ]->shortcutPressed( ItemPtr( this ) );
		return true;
	}
...
bool Item::act()
{
	...
	if( types().find( type() ) != types().end() )
		return types()[ type() ]->act( ItemPtr(this) );
	return false;
}
...
void Item::changed()
{
	...
	Manager::instance().changed( ItemPtr(this) );
}
...
BasicItemType::BasicItemType( const BW::string& typeName ) : name_( typeName )
{
	...
	Item::registerType( ItemTypePtr( this ) );
}
