...
//==============================================================================
Menu::Menu( const std::string& root, IMenuHelper * menuHelper )
...
std::wstring Menu::buildMenuText( ItemPtr item )
{
	...
	BW::wstring text;
	bw_utf8tow( item->displayName().c_str(), text );
	if ( !item->shortcutKey().empty() )
	{
		BW::wstring wshortcutkey;
		bw_utf8tow( item->shortcutKey().c_str(), wshortcutkey );
		text += L"\t" + wshortcutkey;
	}
