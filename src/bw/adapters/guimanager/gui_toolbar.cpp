...
Toolbar::Toolbar( const std::string& root, HWND toolbar, unsigned int iconSize /* = 16 */ ) :
	Subscriber( BW::string( root.c_str() ) ),
	toolbar_( toolbar ),
	forceChanged_( false )
{
	BW_GUARD;
...
int Toolbar::getImageIndex( ItemPtr item )
{
	...
	if( imageIndices_.find( item.get() ) == imageIndices_.end() )
	{
		...
		imageIndices_[ item.get() ] = ( int ) result_;
	}
	return imageIndices_[ item.get() ];
}
...
void Toolbar::updateToggle( unsigned int& index, ItemPtr item, TBBUTTONINFO& info )
{
		...
		info.idCommand != item->commandID() ) // not for me
	{
		...
		button.dwData = (DWORD_PTR)item.get();
		button.iString = -1;
		sendMessage( TB_INSERTBUTTON, index, (LPARAM)&button );
	}
	TBBUTTONINFO buttonInfo = { sizeof( buttonInfo ), TBIF_TEXT | TBIF_BYINDEX | TBIF_STATE };
	...
	const BW::string appTbsSection, const HwndVector& appTbsData, int iconSize /*=16*/ )
{
	...
	{
		Manager::instance().add(  std::dynamic_pointer_cast<GUI::Subscriber>( 
			GUI::SubscriberPtr( new Toolbar( (*appTbsItem)[ i ]->name().c_str(), *hwndIt, iconSize ) ) ) );
	}
