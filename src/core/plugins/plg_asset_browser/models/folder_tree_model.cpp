...
struct FolderTreeModel::Implementation
{
	...
	AssetBrowserPageModel* pageModel_;
...
FolderTreeModel::FolderTreeModel( AssetBrowserPageModel & pageModel )
	: impl_( new Implementation( *this ) )
{
		init( &pageModel );
}
...
void FolderTreeModel::init( AssetBrowserPageModel* pageModel )
{
	impl_->pageModel_ = pageModel;
	impl_->fileSystem_ = pageModel->fileSystem();

	setAssetPaths( pageModel->assetPaths() );
}

AssetBrowserPageModel* FolderTreeModel::pageModel() const
{
	return impl_->pageModel_;
}
...
void FolderTreeModel::setAssetPaths(const std::vector<std::string>& paths)
{
	...
	impl_->pageModel_->populateFolderContents( paths );
}
