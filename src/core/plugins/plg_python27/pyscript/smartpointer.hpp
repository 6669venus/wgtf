...
#define BW_DECLARE_SMARTPOINTER( CLASS )								\
	BW_DECLARE_SMARTPOINTER_WITH_NAME( CLASS, CLASS##Ptr )

#define BW_DECLARE_SMARTPOINTER_WITH_NAME( CLASS, POINTER_CLASS_NAME )	\
	class CLASS; 														\
	typedef SmartPointer< CLASS > POINTER_CLASS_NAME;
...
public:
	 ...
	 *	returns non-zero if the original value was non-zero or
	 *	returns zero if the original value was zero.
	 */
	inline int32 BW_FASTCALL incRefIfNotZero() const
	{
#ifdef SAFE_REFERENCE_COUNT_DEBUG
		if (this == debuggingRef())
		{
		...
		for (;;)
		{
			// load current ref count and return false if zero
			int32 oldValue = count_;
			if (oldValue == 0)
			{
				return 0;
			}

			// attempt compare and swap of incremented value
			int32 newValue = oldValue + 1;
			int32 currentValue = BW_ATOMIC32_COMPARE_AND_SWAP( &count_, newValue, oldValue );
			// if compare and swap succeeded then return, otherwise retry
...
private:
	...
	/**
	 *	This method is called when the SafeReferenceCount reference count of
	 *	this object becomes 0.
	 *
	 *	The default behaviour is 'delete this;', but it can be overridden to
	 *	allow either alternative memory management systems, e.g., deleting
	 *	under the scope of a different global new operator; or to control the
	 *	intersection of multiple lifetimes, e.g., destroy() indicates the last
	 *	SmartPointer<> reference has gone, but the object may now be cached or
	 *	owned by a different system, and so should not be deleted.
	 */
 ...
 */
template <class Ty>
inline int32 incrementReferenceCountIfNotZero( const Ty &Q )
{
	return Q.incRefIfNotZero();
}


template <>
inline int32 incrementReferenceCountIfNotZero<SafeReferenceCount>(
...
public:
	...
	enum TRY_tag { IS_NON_ZERO_REFERENCE };
...
public:
	...
	ConstSmartPointer( const Object *P, TRY_tag ) :
		object_( incrementReferenceCountIfNotZero( *P ) ? P : 0 ) 
	{
		if (object_) 
		{
			BW_MEMORYDEBUG_SMARTPOINTER_ASSIGN(this, object_);
