...
namespace 
{
    ...
    class QtPreferencesImpl : public IEnvEventListener
    {
    public:

	    QtPreferencesImpl( IComponentContext& contextManager, QtPreferences& qtPreferences)
    ...
    GenericObjectPtr & QtPreferencesImpl::getPreference( const char * key )
    {
    ...
    void QtPreferencesImpl::registerPreferencesListener( std::shared_ptr< IPreferencesListener > & listener )
    {
        listeners_.push_back( listener );
    }
    void QtPreferencesImpl::deregisterPreferencesListener( std::shared_ptr< IPreferencesListener > & listener )
    {
        listeners_.erase( listener );
    }

    void QtPreferencesImpl::saveCurrentPreferenceToFile( const char * filePath )
    {
        auto definitionManager = contextManager_.queryInterface< IDefinitionManager >();
        auto fileSystem = contextManager_.queryInterface< IFileSystem >();
    ...
    void QtPreferencesImpl::savePreferenceState(  ISerializer & serializer, const PreferenceEnvCom* ec )
    {
    ...
    void QtPreferencesImpl::loadPreferenceState(  ISerializer & serializer, PreferenceEnvCom* ec )
    {
         std::unique_lock<std::mutex> lock( mutex_ );
         auto definitionManager = contextManager_.queryInterface< IDefinitionManager >();
...
{
    pImpl_.reset( new QtPreferencesImpl( contextManager, *this ) );
}
