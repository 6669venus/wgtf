...
Item {
	...
	Loader {
		...
		onLoaded: {
			...
			{
				var textField = actionHandler.findChildItem(textBoxLoader, "TextField");
				textField.forceActiveFocus();
				
				var textInput = actionHandler.findChildItem(textField, "TextInput");
				textInput.autoScroll = false;
				
				var mappedMouse = mapToItem(textInput, activatePosition.x, activatePosition.y);
				textField.cursorPosition = textInput.positionAt(mappedMouse.x, mappedMouse.y);
